...
wants <- c("tidyverse", "data.table", "DT", "openair", "here", "lubridate", "deweather", "httr", "jsonlite", "ggeasy", "zoo", "DBI", "dbplyr", "sf", "config", "patchwork", "gganimate", "gifski")
...
dateTo = "2020-06-30" #final day of lockdown period (or latest date for monitoring data)
...
con %>% 
    dbDisconnect() %>% 
    rm()
#-------get lat long for DT's--------removed as DT not mapped in later versions
...
qry_list_tc <- list(select = select_str_tc, group_by = groupby_str_tc, where = where_str_tc, apikey = apikey)# apikey is in the query rather than in an authenticate argument
...
count_data <- content(r_tc, as="text") %>% 
    ...
    summarise(sum_daily_flow = sum(flow)) #%>% 
    # summarise(aadt = mean(sum_daily_flow, na.rm = T), .groups = "drop")
...
qry_list_jl <- list(select = select_str_jl, group_by = group_by_str_jl, where = where_str_jl, apikey = apikey)# apikey is in the query rather than in an authenticate argument
...
# mean_jt = mean(avg_jt, na.rm = T), .groups = "drop") 
...
monthly_tube_plot <- tubes_for_map %>%
  ...
  facet_grid(rows = vars(monthname)) +
  theme_ppt_single() +
  theme(panel.grid = element_blank())

#stuff below is for tubes
...
save(aq_data_DT, met_data_DT, aq_monitors, core_means_joined, models_plots, models_plots_sites, monthly_tubes, wards, counts_speeds, file = "S:/SUSTAIN/EnvQual/Air_Quality/Projects/R Projects/air_quality_data_management/data/dfs.RData")
...
dwp_bar_plot <- dwp_site_data %>% #Weekly Bar Plot
timeAverage(avg.time = "week", type = c("site", "pollutant")) %>%
  ggplot(aes(x = date, y = pred, fill = pollutant)) +
  geom_col() +
  facet_wrap(~ site) +
  scale_fill_manual(values = c("firebrick", "dodgerblue")) +
  theme_report_facet() +
  labs(x = "Date: 2020", y = quickText("Weekly mean concentrations:  ugm-3"), title = quickText("Weekly de - weathered NOx and NO2"), fill = "Pollutant")
...
#cusum calcs dodgy - need modelled counterfactual
...
```{r Traffic flows, eval = TRUE, echo = FALSE, results = "show", out.width = '100%', fig.width = 14, fig.height = 12, message = FALSE, warnings = FALSE, fig.cap = "Fig 1. Daily traffic flows and speeds: 2020"}
...
```{r Traffic flows by sector, eval = TRUE, echo = FALSE, results = "show", out.width = '100%', fig.width = 14, fig.height = 12, message = FALSE, warnings = FALSE, fig.cap = "Fig 2. Traffic flows by sector: 2020"}

#-------------------------GOOGLE TRAFFIC DATA----------------------
google_mob_data <- fread("https://www.gstatic.com/covid19/mobility/Global_Mobility_Report.csv") %>%
    filter(sub_region_1 == "Bristol City")

sector_mobility_plot <- google_mob_data %>%
    select(date:last_col()) %>% 
    pivot_longer(-date, names_to = "sector", values_to = "pc_change_baseline") %>%
    mutate(date = as.Date(date)) %>% 
    separate(sector, into = c("Sector", "percent"), sep = "_percent_") %>% 
    select(-percent) %>% 
...
```{r aq map, eval = TRUE, echo = FALSE, out.width = '100%', fig.width = 14, fig.height = 12, fig.cap = "Fig 3. Locations of current continuous monitoring sites"}
...
```{r data viz National, eval = TRUE, echo = FALSE, warning = FALSE, results = "show", fig.height = 7, fig.width = 11, fig.cap = "Fig 4. National comparison: nitrogen dioxide"}
#--------------------------------------CORE CITIES-------------------------
...
```{r data viz Colston, eval = TRUE, echo = FALSE, warning = FALSE, results = "show", fig.height = 7, fig.width = 11, fig.cap = "Fig 5. Chart of Colston Avenue site by pollutant"}
...
```{r data viz Bristol sites, eval = TRUE, echo = FALSE, warning = FALSE, results = "show", fig.height = 7, fig.width = 11, fig.cap = "Fig 6. NOx and NO2: Wells Road"}
...
```{r data viz diffusion tubes, eval = TRUE, echo = FALSE, warning = FALSE, results = "show", fig.height = 7, fig.width = 11, fig.cap = "Fig 7. mean concentrations of NO2 in April diffusion tubes by year"}

 # The median value for 2020 was `r round(medians["2020"])` ugm^-3^ which was `r round((medians["2020"] / min(medians[1:9])) * 100)`% of the previous lowest median April value from `r names(which.min(medians[1:9]))`
...
```
```{r data viz timeVariation NOx show, eval = TRUE, echo = FALSE, results = "show", fig.height = 7, fig.width = 11, fig.cap = "Fig 8. Time variation of NOx at roadside sites in lockdown period by hour"}
plot(tv_nox, subset = "hour")
```

A similar reduction is shown when plotted by day of the week.
```{r data viz timeVariation NOx show day, eval = TRUE, echo = FALSE, results = "show", fig.height = 7, fig.width = 11, fig.cap = "Fig 9. Time variation of NOx at roadside sites in lockdown period by day"}
...
```{r data viz timeVariation NO2 show, eval = TRUE, echo = FALSE, results = "show", , fig.height = 7, fig.width = 11, fig.cap = "Fig 10. Time variation of NO2 at roadside sites in lockdown period by hour"}
...
```{r data viz timeVariation PM10 show, eval = TRUE, echo = FALSE, results = "show", fig.height = 7, fig.width = 11, fig.cap = "Fig 11. Time variation of PM10 in lockdown period by hour"}
...
```{r data viz Meteorology, eval = TRUE, echo = FALSE, warning = FALSE, results = "show", fig.height = 7, fig.width = 10, fig.cap = "Fig 12. Wind roses for lockdown periods: Bristol Lulsgate"}
met_data_DT_hr %>% 
  filter(between(yday(date), lockdown_start_julian, lockdown_end_julian)) %>% 
...
```{r table sites, eval = TRUE, echo = FALSE, warning = FALSE, message = FALSE, results = "show", fig.height = 7, fig.width = 13, fig.cap = "Table 3. Sites aggregated for model analysis"}
...
```{r deweather chart, eval = TRUE, echo = FALSE, warning = FALSE, message = FALSE, results = "show", fig.height = 7, fig.width = 14, fig.show = "show", fig.cap = "Fig 15. Deweathered chart by site class"}
...
```{r deweathered predictions of NOx and NO2 by site, eval = TRUE, echo = FALSE, warning = FALSE, message = FALSE, results = "show", fig.height = 7, fig.width = 10, fig.show = "show", fig.cap = "Fig 15. Deweathered chart by site: NOx and NO2"}
...
```{r deweathered weekly NOx and NO2 by site, eval = TRUE, echo = FALSE, warning = FALSE, message = FALSE, results = "show", fig.height = 7, fig.width = 10, fig.show = "show", fig.cap = "Fig. 16. Predicted weekly mean NOx and NO2: De - weathered predictions"}

dwp_bar_plot
```


```{r cusum analysis, eval = TRUE, echo = FALSE, warning = FALSE, message = FALSE, results = "hide", fig.height = 7, fig.width = 10, , fig.show = "hide", fig.cap = "Fig 17. Animated de - weathered NOx data"}
